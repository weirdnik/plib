...
YOUTUBE_RE = re.compile ('https?://(www.)?((?:youtu.be/)|(?:youtube.com/watch\?(?:feature=player_embedded&(?:amp;)?)?v=))(?P<video>[\w\d-]+)(?:#t=\d+)?(?:&feature=youtu.be)?')
...
PROCESS_RE = re.compile('(?P<YT>https?://(www.)?((?:youtu.be/)|(?:youtube.com/watch\?(?:feature=player_embedded&(?:amp;)?)?v=))(?P<video>[\w\d-]+)(?:#t=\d+)?(?:&feature=youtu.be)?)|(?P<Vimeo>https?://(www.)?vimeo.com/(?P<V_ID>[\w\d]+))|(?P<Instagram>https?://instagram.com/p/(?P<I_ID>[\w\d]+)/?)|(?<!(<iframe src=")|(t="270" src="))(?P<url>https?://[\w-]+(\.[\w-]+)*(/[\w\?=,.\-%&;#]*)*)')
